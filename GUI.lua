...
local function unicodeFind(str, pattern, init, plain)
	if init then
		if init < 0 then
	...
	local a, b = string.find(str, pattern, init, plain)
...
local function stringWrap(data, limit)
	...
	while i <= #wrappedLines do
		local position = unicodeFind(wrappedLines[i], "\n")
		if position then
...
local function stringLimit(s, limit, mode, noDots)
...
local function filesystemExtension(path, lower)
	local extension = path:match("[^%/]+(%.[^%/]+)%/?$")
	return (lower and extension) and (unicode.lower(extension)) or extension
end

local function numberRound(num) 
	if num >= 0 then
...
local function numberRoundToDecimalPlaces(num, decimalPlaces)
	local mult = 10 ^ (decimalPlaces or 0)
	return numberRound(num * mult) / mult
end

local function numberShorten(num, digitCount)
	if num < 1000 then
		return num
	else
		...
		return numberRoundToDecimalPlaces(num / 1000 ^ index, digitCount) .. shortcuts[index]
	end
end
...
local GUI = {
	...
	PALETTE_CONFIG_PATH = "/etc/.gui_palette.cfg",
...
function GUI.container(x, y, width, height)
	...
	container.children = {}
	container.passbufferEvents = true
...
local function workspaceStart(workspace, eventPullTimeout)
	...
	local function handle(isbufferEvent, currentContainer, intersectionX1, intersectionY1, intersectionX2, intersectionY2)
		if
			not isbufferEvent or
			intersectionX1 and
		...
		then
			...
			if isbufferEvent then
				...
				currentContainerPassed = not currentContainer.passbufferEvents
					...
					if child.children then
						...
						if isbufferEvent then
							if child:isPointInside(e3, e4) then
								...
								if not child.passbufferEvents then
									return true
								end
							end
	...
	repeat
		e1, e2, e3, e4, e5, e6, e7, e8, e9, e10, e11, e12, e13, e14, e15, e16, e17, e18, e19, e20, e21, e22, e23, e24, e25, e26, e27, e28, e29, e30, e31, e32 = computer.pullSignal(workspace.animations and 0 or workspace.eventPullTimeout)
...
function GUI.alert(...)
	...
	lines = stringWrap(lines, textWidth)
...
function GUI.codeView(x, y, width, height, fromSymbol, fromLine, maximumLineLength, selections, highlights, syntaxPatterns, syntaxColorScheme, syntaxHighlight, lines)	
	...
	codeView.passbufferEvents = false
...
local function colorSelectorDraw(colorSelector)
	...
	buffer.drawText(colorSelector.x + 1, colorSelector.y + math.floor(colorSelector.height / 2), overlayColor, stringLimit(colorSelector.text, colorSelector.width - 2))
...
local function getAxisValue(number, postfix, roundValues)
	if roundValues then
		...
		if math.abs(integer) >= 1000 then
			return numberShorten(integer, 2) .. postfix
		else
...
local function sliderDraw(object)
	...
	if object.showMaximumAndMinimumValues then
		local stringMaximumValue, stringMinimumValue = tostring(object.roundValues and math.floor(object.maximumValue) or numberRoundToDecimalPlaces(object.maximumValue, 2)), tostring(object.roundValues and math.floor(object.minimumValue) or numberRoundToDecimalPlaces(object.minimumValue, 2))
	...
	if object.currentValuePrefix or object.currentValuePostfix then
		local stringCurrentValue = (object.currentValuePrefix or "") .. (object.roundValues and math.floor(object.value) or numberRoundToDecimalPlaces(object.value, 2)) .. (object.currentValuePostfix or "")
	...
	local activeWidth = numberRound((object.value - object.minimumValue) / (object.maximumValue - object.minimumValue) * object.width)
	buffer.drawText(object.x, object.y, object.colors.passive, string.rep("━", object.width))
	buffer.drawText(object.x, object.y, object.colors.active, string.rep("━", activeWidth))
		...
		switch:addAnimation(
			function(animation)
				if switch.state then
					switch.pipePosition = numberRound(1 + animation.position * (switch.width - 2))
				else	
					switch.pipePosition = numberRound(1 + (1 - animation.position) * (switch.width - 2))
				end
			end,
			function(animation)
				animation:remove()
		...
		if not child.hidden then
			...
			if layoutRow >= 1 and layoutRow <= #layout.rowSizes and layoutColumn >= 1 and layoutColumn <= #layout.columnSizes then
				...
				if cell.horizontalFitting then
					child.width = numberRound(layout.columnSizes[layoutColumn].calculatedSize - cell.horizontalFittingRemove)
				end

				if cell.verticalFitting then
					child.height = numberRound(layout.rowSizes[layoutRow].calculatedSize - cell.verticalFittingRemove)
				end
		...
		for j = 1, #layout.columnSizes do
			for i = 1, #layout.rowSizes do
				buffer.drawFrame(
					numberRound(x),
					numberRound(y),
					numberRound(layout.columnSizes[j].calculatedSize),
					numberRound(layout.rowSizes[i].calculatedSize),
					0xFF0000
				)
...
local function filesystemChooserDraw(object)
	...
	buffer.drawText(object.x + 1, y, object.colors.text, stringLimit(object.path or object.placeholderText, object.width - tipWidth - 2, "left"))
...
local function filesystemTreeUpdateFileListRecursively(tree, path, offset)
	...
	if tree.showMode == GUI.IO_MODE_BOTH or tree.showMode == GUI.IO_MODE_FILE then
		for i = 1, #list do
			tree:addItem(list[i], path .. list[i], offset, false, tree.extensionFilters and not tree.extensionFilters[filesystemExtension(path .. list[i], true)] or false)
		end
	end
end
	...
	if object.autoWrap then
		for i = 1, #object.lines do
			...
			for subLine in (isTable and object.lines[i].text or object.lines[i]):gmatch("[^\n]+") do
				local wrappedLine = stringWrap(subLine, object.textWidth)
	...
	for i = object.currentLine, object.currentLine + object.textHeight - 1 do
		if object.linesCopy[i] then
			...
			if lineType == "string" then
				line, textColor = stringLimit(object.linesCopy[i], object.textWidth), object.colors.text
			elseif lineType == "table" then
				line, textColor = stringLimit(object.linesCopy[i].text, object.textWidth), object.linesCopy[i].color
			else
			...
			buffer.drawText(math.floor(x), y, textColor, line)
			y = y + 1
		else
			break
		end
...
function GUI.palette(x, y, startColor)
	...
	bigCrest.passbufferEvents = true
	...
	if filesystem.exists(GUI.PALETTE_CONFIG_PATH) then
		local file = io.open(GUI.PALETTE_CONFIG_PATH, "r")
		favourites = serialization.unserialize(file:read("*a"))
		file:close()
	else
		favourites = {}
		for i = 1, 6 do favourites[i] = color.HSBToInteger(math.random(0, 360), 1, 1) end
		local file = io.open(GUI.PALETTE_CONFIG_PATH, "w")
		file:write(serialization.serialize(favourites))
		file:close()
	end
	...
	bigImage.eventHandler = function(workspace, object, e1, e2, e3, e4)
		if e1 == "touch" or e1 == "drag" then
			...
			paletteSwitchColorFromHex(select(3, buffer.getGPUProxy().get(e3, e4)))
			workspace:draw()
		end
	end
...
function GUI.highlightString(x, y, fromChar, indentationWidth, patterns, colorScheme, s)	
	local stringLength, x1, y1, x2, y2 = unicode.len(s), buffer.getDrawLimit()
	...
	local counter, symbols, colors, bufferIndex, newFrameBackgrounds, newFrameForegrounds, newFrameSymbols, searchFrom, starting, ending = indentationWidth, {}, {}, buffer.getIndex(x, y), buffer.getNewFrameTables()
	...
	for j = 1, #patterns, 4 do
		...
		while true do
			starting, ending = unicodeFind(s, patterns[j], searchFrom)
...
local function comboBoxDraw(object)
	...
	if object.dropDownMenu.itemsContainer.children[object.selectedItem] then
		buffer.drawText(object.x + 1, math.floor(object.y + object.height / 2), object.colors.default.text, stringLimit(object.dropDownMenu.itemsContainer.children[object.selectedItem].text, object.width - object.height - 2, "right"))
	end
...
local function windowCheck(window, x, y)
	...
	for i = #window.children, 1, -1 do
		...
		then
			if not child.passbufferEvents and child.eventHandler then
				return true
...
local function tableHeaderDraw(self)
	buffer.drawRectangle(self.x, self.y, self.width, self.height, self.parent.colors.headerBackground, self.parent.colors.headerText, " ")
	buffer.drawText(self.x + 1, self.y, self.parent.colors.headerText, self.text)
end
...
function GUI.tableCellEventHandler(workspace, self, e1, e2, e3, e4, e5, ...)
	if e1 == "touch" or e1 == "drag" or e1 == "double_touch" then
		...
		if (e5 == 0 or not self.parent.selectedRows[row]) and not (keyboard.isControlDown() or keyboard.isKeyDown(219)) then
...
function GUI.tableCellDraw(self)
	...
	if background then
		buffer.drawRectangle(self.x, self.y, self.width, self.height,
			background,
			foreground,
		" ")
	end
...
local function tableTextCellDraw(self)
	buffer.drawText(self.x + 1, self.y, GUI.tableCellDraw(self), self.text)
end
...
local function tableDraw(self)
	-- Items background
	buffer.drawRectangle(self.x, self.y + self.itemHeight, self.width, self.height - self.itemHeight, self.colors.background, 0x0, " ")
	-- Content
	layoutDraw(self)
end
